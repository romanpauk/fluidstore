...
namespace crdt
{
    template < typename Key, typename Value, typename Traits, bool IsEmbeddable = Traits::template TypeTraits< Value >::embeddable > class map_g_base;
    ...
    public:
        ...
        template < typename K > Value get_value(K&& key) const
        {
            return Value(allocator_type(allocator_, ""));
        }
