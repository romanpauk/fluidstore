...
namespace crdt
{
    template < typename Key, typename Value, typename Traits > class map_or
    {
    public:
        ...
        struct map_or_tags
        {
            map_or_tags(allocator_type allocator)
                : added(allocator_type(allocator, "added"))
                , removed(allocator_type(allocator, "removed"))
                // , value(allocator_type(allocator, "value"))
                , allocator_(allocator)
            {}

            map_or_tags(map_or_tags&& other)
            ...
            map_or_tags& operator = (map_or_tags&& other)
            {
        ...
        {
        public:
            iterator_base(map_or< Key, Value, Traits >& map, It&& it)
                : it_(std::move(it))
                , map_(map)
            {}
            ...
            bool operator != (const iterator_base< It >& other) const { return it_ != other.it_; }
            const auto& operator*() { return it_->first; }
            ...
            iterator_base< It >& operator++()
            {
                ...
                {
                    ++it_;
                } while (*this != map_.end() && !map_.is_added(it_->second));

                return *this;
            }

        private:
            map_or< Key, Value, Traits >& map_;
            typename It it_;
        };

    public:
        typedef typename Traits::template Map< Key, map_or_tags > container_type;

        map_or(allocator_type allocator)
            : allocator_(allocator)
        ...
        template < typename K, typename V > std::pair< iterator, bool > insert(K&& key, V&& value)
        {
            auto pairb = values_.emplace(
                std::forward< K >(key),
                allocator_.template create< map_or_tags >()
                );

            pairb.first->second.added.insert(get_tag());
            pairb.first->second.value = std::forward< V >(value);
